theory LRE_Beh_24122022_110829
...
zoperation InitialToOCM =
  ...
  update "[st\<Zprime>= OCM
         ,tr\<Zprime> =tr @ [Event position] @ [State OCM]
         ,trg\<Zprime> = None
...
zoperation OCMToMOM =
  over LRE_Beh
  pre "st= OCM \<and> ( + )\<le> \<and> dist(pos, Obsts)>10 \<and> \<not>inOPEZ(pos) \<and> trg=Some reqMOM"
  update "[st\<Zprime>= MOM
          , vel\<Zprime> =setVel(vel, MOMVel)
         ,tr\<Zprime> =tr @ [Event reqMOM]@ [Event advVel] @ [State MOM]
         ,trg\<Zprime> = None
         ,triggers\<Zprime> = {reqOCM, endTask}
         ]"
...
zoperation MOMToOCM =
  ...
  update "[st\<Zprime>= OCM
         ,tr\<Zprime> =tr @ [Event reqOCM]@ [Event position] @ [State OCM]
         ,trg\<Zprime> = None
         ,triggers\<Zprime> = {reqVel, reqMOM}
         ]"
...
zoperation MOMToHCM =
  over LRE_Beh
  pre "st= MOM \<and> ( + )> \<and> dist(pos, Obsts)\<le>StaticObsDist \<and> CDA(pos, Obsts, vel)>MinSafeDist \<and> \<not>inOPEZ(pos)"
...
zoperation OCMToOCM =
  over LRE_Beh
  params reqV \<in> "integer\<times>integer" 
  pre "st= OCM \<and> trg=Some reqVel"
  update "[st\<Zprime>= OCM
         
         ,tr\<Zprime> =tr @ [Event reqVel]@ [Event advVel]@ [Event position] @ [State OCM]
         ,trg\<Zprime> = None
         ,triggers\<Zprime> = {reqVel, reqMOM}
         ]"
...
zoperation MOMToOCM_2 =
  ...
  update "[st\<Zprime>= OCM
         ,tr\<Zprime> =tr @ [Event endTask]@ [Event advVel]@ [Event position] @ [State OCM]
         ,trg\<Zprime> = None
         ,triggers\<Zprime> = {reqVel, reqMOM}
         ]"
...
zoperation CAMToOCM =
  ...
  update "[st\<Zprime>= OCM
         ,tr\<Zprime> =tr @ [Event advVel]@ [Event position] @ [State OCM]
         ,trg\<Zprime> = None
         ,triggers\<Zprime> = {reqVel, reqMOM}
         ]"
...
zoperation MOMToCAM_1 =
  over LRE_Beh
  pre "st= MOM \<and> (((( + )<0 \<or> ( + )>99) \<or> ( + )<0) \<or> ( + )>99)"
  update "[st\<Zprime>= CAM
          , vel\<Zprime> =maneuv(vel)
         ,tr\<Zprime> =tr @ [Event advVel] @ [State CAM]
         ,trg\<Zprime> = None
...
zoperation HCMToCAM_1 =
  over LRE_Beh
  pre "st= HCM \<and> (((( + )<0 \<or> ( + )>99) \<or> ( + )<0) \<or> ( + )>99)"
  update "[st\<Zprime>= CAM
          , vel\<Zprime> =maneuv(vel)
         ,tr\<Zprime> =tr @ [Event advVel] @ [State CAM]
         ,trg\<Zprime> = None
...
zoperation CAMToCAM_1 =
  over LRE_Beh
  pre "st= CAM \<and> (((( + )<0 \<or> ( + )>99) \<or> ( + )<0) \<or> ( + )>99)"
  update "[st\<Zprime>= CAM
          , vel\<Zprime> =maneuv(vel)
         ,tr\<Zprime> =tr @ [Event advVel] @ [State CAM]
         ,trg\<Zprime> = None
...
lemma InitialToOCM_inv [hoare_lemmas]: "InitialToOCM (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma OCMToMOM_inv [hoare_lemmas]: "OCMToMOM() preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma MOMToOCM_inv [hoare_lemmas]: "MOMToOCM (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma HCMToOCM_inv [hoare_lemmas]: "HCMToOCM (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma MOMToHCM_inv [hoare_lemmas]: "MOMToHCM() preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma MOMToOCM_1_inv [hoare_lemmas]: "MOMToOCM_1 (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma HCMToOCM_1_inv [hoare_lemmas]: "HCMToOCM_1 (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma HCMToMOM_inv [hoare_lemmas]: "HCMToMOM() preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma OCMToOCM_inv [hoare_lemmas]: "OCMToOCM (reqV, pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma MOMToOCM_2_inv [hoare_lemmas]: "MOMToOCM_2 (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
...
lemma CAMToOCM_inv [hoare_lemmas]: "CAMToOCM (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma CAMToOCM_1_inv [hoare_lemmas]: "CAMToOCM_1 (pos) preserves LRE_Beh_inv"
  by (zpog_full; auto)
...
lemma "InitialToOCM (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "OCMToMOM() preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "MOMToOCM (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "HCMToOCM (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "MOMToHCM() preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "MOMToOCM_1 (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "HCMToOCM_1 (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "HCMToMOM() preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "OCMToOCM (reqV, pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "MOMToOCM_2 (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
...
lemma "CAMToOCM (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
  
lemma "CAMToOCM_1 (pos) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
