theory LRE_Beh_26122022_025258
...
zoperation OCMToMOM =
  over LRE_Beh
  pre "st= OCM \<and> ((vel.1 * vel.1) + (vel.2 * vel.2))\<le>(3 * 3) \<and> dist(pos, Obsts)>10 \<and> \<not>inOPEZ(pos) \<and> trg=Some reqMOM"
...
zoperation MOMToOCM_2 =
  ...
  update "[st\<Zprime>= OCM
         , vel\<Zprime> = (IntegerExp [value=0], IntegerExp [value=0])
