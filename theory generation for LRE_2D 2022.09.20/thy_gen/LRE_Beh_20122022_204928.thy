theory LRE_Beh_20122022_084928
...
definition "St = {OCM, MOM, HCM, CAM, initial}" 


enumtype Evt = enumtype St = OCM | MOM | HCM | CAM | initial advHdng 
definition "Evt = {definition "St = {OCM, MOM, HCM, CAM, initial}"advHdng}" 
...
zoperation InitialToOCM =
  ...
  update "[ st\<Zprime>= OCM
       ,tr\\<Zprime> =tr @ @ [Event position]@ [Event velocity] @ OCM
         ,trg\\<Zprime> = None
          ]"
...
zoperation OCMToMOM =
  ...
  update "[ st\<Zprime>= MOM
       ,tr\\<Zprime> =tr @ @ [Event reqMOM]@ [Event advVel] @ MOM
         ,trg\\<Zprime> = None
          ]"
...
zoperation MOMToOCM =
  ...
  update "[ st\<Zprime>= OCM
       ,tr\\<Zprime> =tr @ @ [Event reqOCM]@ [Event position]@ [Event velocity] @ OCM
         ,trg\\<Zprime> = None
          ]"
...
zoperation MOMToHCM_1 =
  ...
  update "[ st\<Zprime>= HCM
       ,tr\\<Zprime> =tr @ @ [Event reqHCM]@ [Event advVel] @ HCM
         ,trg\\<Zprime> = None
          ]"
...
zoperation OCMToOCM =
  ...
  update "[ st\<Zprime>= OCM
       ,tr\\<Zprime> =tr @ @ [Event reqVel]@ [Event advVel]@ [Event position]@ [Event velocity] @ OCM
         ,trg\\<Zprime> = None
          ]"
...
zoperation OCMToOCM_1 =
  ...
  update "[ st\<Zprime>= OCM
       ,tr\\<Zprime> =tr @ @ [Event reqHdng]@ [Event advHdng]@ [Event position]@ [Event velocity] @ OCM
         ,trg\\<Zprime> = None
          ]"
        
zoperation MOMToOCM_1_2 =
  over LRE_Beh
  params pos \<in> "Coord" vel \<in> "real" 
  pre "st= MOM \<and> trg=Some endTask"
  update "[ st\<Zprime>= OCM
       ,tr\\<Zprime> =tr @ @ [Event endTask]@ [Event advVel]@ [Event position]@ [Event velocity] @ OCM
         ,trg\\<Zprime> = None
          ]"
...
zoperation HCMToCAM =
  ...
  update "[ st\<Zprime>= CAM
       ,tr\\<Zprime> =tr @ @ [Event maneuver] @ CAM
         ,trg\\<Zprime> = None
          ]"
...
zmachine LRE_BehMachine =
  ...
  operations  InitialToOCM OCMToMOM MOMToOCM HCMToOCM MOMToHCM MOMToHCM_1 MOMToOCM_1 HCMToOCM_1 HCMToMOM OCMToOCM OCMToOCM_1 MOMToOCM_1_2 HCMToCAM MOMToCAM CAMToOCM CAMToOCM_1 
...
lemma MOMToOCM_1_2_inv [hoare_lemmas]: "MOMToOCM_1_2 (pos, vel) preserves LRE_Beh_inv"
  by (zpog_full; auto)
...
lemma "MOMToOCM_1_2 (pos, vel) preserves R1 under LRE_Beh_inv"
  by (zpog_full; auto)
